...
class AddQuickMediaCodeAdmin {
	...
	function settings_link($links, $file) {
		...
		if ( empty($this_plugin) ) {
			$this_plugin = ADDQUICKMEDIACODE_PLUGIN_BASE_FILE;
		}
		if ( $file == $this_plugin ) {
	...
	function plugin_options() {
		...
		<div class="wrap">
			...
			<div id="addquickmediacode-tabs">
					...
					if (!empty($records)) {
						...
						foreach ( $records as $record ) {
							...
							<div style="border-bottom: 1px solid; padding-top: 5px; padding-bottom: 5px;">
								...
								<div><?php echo do_shortcode(stripslashes($record->code)); ?></div>
								</div>
							</div>
							<?php
						}
				...
				<div class="wrap">
					<?php
					$plugin_datas = get_file_data( ADDQUICKMEDIACODE_PLUGIN_BASE_DIR.'/addquickmediacode.php', array('version' => 'Version') );
					$plugin_version = __('Version:').' '.$plugin_datas['version'];
					?>
					<h4 style="margin: 5px; padding: 5px;">
					<?php echo $plugin_version; ?> |
					<a style="text-decoration: none;" href="https://wordpress.org/support/plugin/add-quick-media-code" target="_blank"><?php _e('Support Forums') ?></a> |
					<a style="text-decoration: none;" href="https://wordpress.org/support/view/plugin-reviews/add-quick-media-code" target="_blank"><?php _e('Reviews', 'add-quick-media-code') ?></a>
					</h4>
					<div style="width: 250px; height: 170px; margin: 5px; padding: 5px; border: #CCC 2px solid;">
					<h3><?php _e('Please make a donation if you like my work or would like to further the development of this plugin.', 'add-quick-media-code'); ?></h3>
	...
	function db_delete(){
		...
		if(isset($_POST['addquickmediacode-deletes'])){
			$addquickmediacode_deletes = $_POST['addquickmediacode-deletes'];
		} else {
			return;
		}
